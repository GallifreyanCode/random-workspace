...
public class UserServiceIntegrationTest extends AbstractIntegrationTest {
	...
	public final void testRemoveUser() throws Exception {
		...
		PreparedStatement pStmt = conn
				.prepareStatement("SELECT COUNT(1) FROM USERS WHERE USERID=?");
	...
	public final void testModifyPassword() throws Exception {
		...
		ModifyPasswordRequest request = new ModifyPasswordRequest(
				TEST_PASSWORD, password, password);
		userService.modifyPassword(request);
		login.logout();

		char[] expectedDigestInChars = PasswordUtility.getDigest(
				"password123".toCharArray(), "SHA-512");
	...
	public final void testModifyIncorrectPassword() throws Exception {
		...
		ModifyPasswordRequest request = new ModifyPasswordRequest(
				incorrectOriginalPassword, password, password);
		userService.modifyPassword(request);
		login.logout();
	...
	@Test(expected = ModifyPasswordException.class)
	public final void testModifyPasswordIncorrectConfirmation()
			throws Exception {
		logger.info("About to execute test method {}",
				testMethod.getMethodName());
		User user = new User(TEST_USER_ID, TEST_PASSWORD);
